#!/usr/bin/env bash
# Round-26: the return end of the frontier under the new exit policy.
#
# Rounds 23-25 established the compliant point (c2).  The return end had only
# been measured with the old 3.5% target (r120: 11.22% CAGR, 80.5% win at 14.5%
# volatility), so the two winning exit settings are applied there too.
set -u
...
BASE="--composite combo3 --max-new 1 --max-weight 0.08 --max-positions 12 --risk-per-trade 0.016 --extreme-exit 0 --extreme-liquidates 0 --drawdown-tiers 0.08:0.7,0.12:0.45 --rank-blend 0.5 --use-market-timing 0"
...
for S in 42 7 2024 11 23 101 777 31337 5; do
  ...
  run v2_z1_$S $P9 "${BASE} --vol-target 0.14 --profit-target 0.075 --hold 120"
  run v2_z2_$S $P9 "${BASE} --vol-target 0.14 --profit-target 0.09 --hold 90"
done
echo ALLDONE
