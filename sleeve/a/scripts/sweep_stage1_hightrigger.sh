#!/usr/bin/env bash
# Stage-1 round 3b: does the profit target fire on an intraday touch?
#
# A resting limit order fills as soon as the price reaches it, so triggering on the
# daily high is the realistic semantic; the current rule waits for the close to
# exceed the target.  Whether it helps is empirical: more winners are banked, but
# the fill still happens at the next close, which can be below the target.
set -u
...
# zero control: the flag off must reproduce the headline exactly
run v2_hi_off   --profit-trigger-high 0
run v2_hi_on    --profit-trigger-high 1
run v2_hi_on_t5 --profit-trigger-high 1 --profit-target 0.05
run v2_hi_on_t10 --profit-trigger-high 1 --profit-target 0.10
echo ALLDONE
